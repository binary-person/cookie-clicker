...
*/
function _N(s) { return s; }
function _(s) {
  var v;
  if (typeof(translations)!=='undefined' && (v=translations[s])!==undefined) return v;
  return s;
}
function fmt(s) {
	var args = arguments;
	s = s.replace(/\{(\d+)\}/g,function(m,m1){ return args[(m1|0)+1]; });
	return s;
}
...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			var save=prompt(_('Copy this text and keep it somewhere safe!'),Game.WriteSave(1));
		}
		Game.ImportSave=function()
		{
			var save=prompt(_('Please paste in the text that was given to you on save export.'),'');
		...
		Game.WriteSave=function(exporting)//guess what we'e using to save the game?
		{
			...
			else
			{
				...
				if (document.cookie.indexOf('CookieClickerGame')<0) Game.Popup(_('Error while saving.<br>Export your save instead!'));
				else Game.Popup(_('Game saved'));
			}
		}
		...
		Game.Reset=function(bypass)
		{
			if (bypass || confirm('Do you REALLY want to start over?\n(your will lose your progress, but you will keep your achievements and your prestige.)'))
			{
				...
				if (!bypass)
				{
					...
					if (prestige!=0) Game.Popup(fmt(_('You earn {0} heavenly chip{1}!'),prestige,(prestige==1?'':_('s'))));
				}
			}
		}
		...
		Game.goldenCookie.click=function()
		{
			if (Game.goldenCookie.life>0)
			{
				...
				if (choice=='frenzy')
				{
					...
					Game.frenzyPower=7;
					Game.recalculateGains=1;
					Game.Popup(fmt(_('Frenzy : cookie production x7 for {0} seconds!'),time));
				}
				else if (choice=='multiply cookies')
				{
					...
					Game.Earn(moni);
					Game.Popup(fmt(_('Lucky! +{0} cookies!'),Beautify(moni)));
				}
				else if (choice=='ruin cookies')
				{
					...
					Game.Spend(moni);
					Game.Popup(fmt(_('Ruin! Lost {0} cookies!'),Beautify(moni)));
				}
				else if (choice=='blood frenzy')
				{
					...
					Game.recalculateGains=1;
					Game.Popup(fmt(_('Elder frenzy : cookie production x666 for {0} seconds!'),time));
				}
				else if (choice=='clot')
				{
					...
					Game.recalculateGains=1;
					Game.Popup(fmt(_('Clot : cookie production halved for {0} seconds!'),time));
				}
				else if (choice=='click frenzy')
				{
					...
					Game.recalculateGains=1;
					Game.Popup(fmt(_('Click frenzy! Clicking power x777 for {0} seconds!'),time));
				}
				else if (choice=='chain cookie')
				{
					...
					moni=parseInt(moni);
					Game.Popup(fmt(_('Cookie chain : +{0} cookies!'),Beautify(moni)));
		...
		Game.sayTime=function(time,detail)
		{
			...
			if (time>=Game.fps*60*60*24*2 && detail<2) str=Beautify(time/(Game.fps*60*60*24))+_(' days');
			else if (time>=Game.fps*60*60*24 && detail<2) str=_('1 day');
			else if (time>=Game.fps*60*60*2 && detail<3) str=Beautify(time/(Game.fps*60*60))+_(' hours');
			else if (time>=Game.fps*60*60 && detail<3) str=_('1 hour');
			else if (time>=Game.fps*60*2 && detail<4) str=Beautify(time/(Game.fps*60))+_(' minutes');
			else if (time>=Game.fps*60 && detail<4) str=_('1 minute');
			else if (time>=Game.fps*2 && detail<5) str=Beautify(time/(Game.fps))+_(' seconds');
			else if (time>=Game.fps && detail<5) str=_('1 second');
			return str;
		}
		...
		Game.UpdateMenu=function()
		{
			...
			{
				str+=_('<div class="section">Menu</div>')+
				'<div class="subsection">'+
				_('<div class="title">General</div>')+
				_('<div class="listing"><a class="option" onclick="Game.WriteSave();">Save</a><label>Save manually (the game autosaves every 60 seconds)</label></div>')+
				_('<div class="listing"><a class="option" onclick="Game.ExportSave();">Export save</a><a class="option" onclick="Game.ImportSave();">Import save</a><label>You can use this to backup your save or to transfer it to another computer</label></div>')+
				//_('<div class="listing"><span class="warning" style="font-size:12px;">[Note : importing saves from earlier versions than 1.0 will be disabled beyond September 1st, 2013.]</span></div>')+
				_('<div class="listing"><a class="option warning" onclick="Game.Reset();">Reset</a><label>Reset your game (you will keep your achievements)</label></div>')+
				_('<div class="listing"><a class="option warning" onclick="Game.HardReset();">Wipe save</a><label>Delete all your progress, including your achievements and prestige</label></div>')+
				_('<div class="title">Settings</div>')+
				_('<div class="listing">')+
				Game.WriteButton('fancy','fancyButton',_('Fancy graphics ON'),_('Fancy graphics OFF'),'Game.ToggleFancy();')+
				Game.WriteButton('particles','particlesButton',_('Particles ON'),_('Particles OFF'))+
				Game.WriteButton('numbers','numbersButton',_('Numbers ON'),_('Numbers OFF'))+
				Game.WriteButton('milk','milkButton',_('Milk ON'),_('Milk OFF'))+
				'</div>'+
				'<div class="listing">'+Game.WriteButton('autoupdate','autoupdateButton',_('Offline mode OFF'),_('Offline mode ON'))+_('<label>(note : this disables update notifications)</label></div>')+
				//'<div class="listing">'+Game.WriteButton('autosave','autosaveButton',_('Autosave ON'),_('Autosave OFF'))+'</div>'+
				'</div>'
				;
			}
			if (Game.onMenu=='log')
			...
			else if (Game.onMenu=='stats')
			{
				...
				for (var i in list)
				{
					...
					if (!Game.Has('Neuromancy'))
					{
						if (me.bought>0 && me.hide!=3)
						{
							...
							'<div style="min-width:200px;"><div style="float:right;"><span class="price">'+Beautify(Math.round(me.basePrice))+'</span></div><small>'+_('[Upgrade]')+_(' [Purchased]')+'</small><div class="name">'+_(me.name)+'</div><div class="description">'+me.desc+'</div></div>'
					...
					else
					{
						...
						'<div style="min-width:200px;"><div style="float:right;"><span class="price">'+Beautify(Math.round(me.basePrice))+'</span></div><small>'+_('[Upgrade]')+(me.bought>0?_(' [Purchased]'):'')+'</small><div class="name">'+_(me.name)+'</div><div class="description">'+me.desc+'</div></div>'
						,0,0,'bottom-right')+' style="background-position:'+(-me.icon[0]*48+6)+'px '+(-me.icon[1]*48+6)+'px;"></div>';
						upgradesOwned++;
					}
				...
				for (var i in list)
				{
					...
					if (me.won>0 && me.hide==3)
					{
						...
						'<div style="min-width:200px;"><small>'+_('[Achievement]')+_(' [Unlocked]')+(me.hide==3?' [Shadow]':'')+'</small><div class="name">'+_(me.name)+'</div><div class="description">'+me.desc+'</div></div>'
						,0,0,'bottom-right')+' style="background-position:'+(-me.icon[0]*48+6)+'px '+(-me.icon[1]*48+6)+'px;"></div>';
						achievementsOwned++;
					}
					else if (me.won>0)
					...
					{//onclick="Game.Win(\''+me.name+'\');" 
						...
						'<div style="min-width:200px;"><small>'+_('[Achievement]')+'</small><div class="name">'+_(me.name)+'</div><div class="description">'+me.desc+'</div></div>'
						,0,0,'bottom-right')+' style="background-position:'+(-me.icon[0]*48+6)+'px '+(-me.icon[1]*48+6)+'px;"></div>';
					}
					else if (me.hide==1)
					{//onclick="Game.Win(\''+me.name+'\');" 
						achievements+='<div class="crate achievement" '+Game.getTooltip(
						'<div style="min-width:200px;"><small>'+_('[Achievement]')+'</small><div class="name">'+_(me.name)+'</div><div class="description">???</div></div>'
						,0,0,'bottom-right')+' style="background-position:'+(-0*48+6)+'px '+(-7*48+6)+'px;"></div>';
					}
					else if (me.hide==2)
					{//onclick="Game.Win(\''+me.name+'\');" 
						achievements+='<div class="crate achievement" '+Game.getTooltip(
						'<div style="min-width:200px;"><small>'+_('[Achievement]')+'</small><div class="name">???</div><div class="description">???</div></div>'
						,0,0,'bottom-right')+' style="background-position:'+(-0*48+6)+'px '+(-7*48+6)+'px;"></div>';
					}
				}
				var milkName=_('plain milk');
				if (Game.milkProgress>=2.5) milkName=_('raspberry milk');
				else if (Game.milkProgress>=1.5) milkName=_('chocolate milk');
				
				var researchStr=Game.sayTime(Game.researchT);
				var pledgeStr=Game.sayTime(Game.pledgeT);
				var wrathStr='';
				if (Game.elderWrath==1) wrathStr=_('awoken');
				else if (Game.elderWrath==2) wrathStr=_('displeased');
				else if (Game.elderWrath==3) wrathStr=_('angered');
				else if (Game.elderWrath==0 && Game.pledges>0) wrathStr=_('appeased');
				...
				str+=_('<div class="section">Statistics</div>')+
				'<div class="subsection">'+
				_('<div class="title">General</div>')+
				_('<div class="listing"><b>Cookies in bank :</b> <div class="price plain">')+Beautify(Game.cookies)+'</div></div>'+
				_('<div class="listing"><b>Cookies baked (all time) :</b> <div class="price plain">')+Beautify(Game.cookiesEarned)+'</div></div>'+
				(Game.cookiesReset>0?_('<div class="listing"><b>Cookies forfeited by resetting :</b> <div class="price plain">')+Beautify(Game.cookiesReset)+'</div></div>':'')+
				_('<div class="listing"><b>Game started :</b> ')+date+_(' ago</div>')+
				_('<div class="listing"><b>Buildings owned :</b> ')+Beautify(buildingsOwned)+'</div>'+
				_('<div class="listing"><b>Cookies per second :</b> ')+Beautify(Game.cookiesPs,1)+_(' <small>(multiplier : ')+Beautify(Math.round(Game.globalCpsMult*100),1)+_('%)</small></div>')+
				_('<div class="listing"><b>Cookies per click :</b> ')+Beautify(Game.computedMouseCps,1)+'</div>'+
				_('<div class="listing"><b>Cookie clicks :</b> ')+Beautify(Game.cookieClicks)+'</div>'+
				_('<div class="listing"><b>Hand-made cookies :</b> ')+Beautify(Game.handmadeCookies)+'</div>'+
				_('<div class="listing"><b>Golden cookie clicks :</b> ')+Beautify(Game.goldenClicks)+'</div>'+//_(' <span class="hidden">(<b>Missed golden cookies :</b> ')+Beautify(Game.missedGoldenClicks)+_(')</span></div>')+
				_('<br><div class="listing"><b>Running version :</b> ')+Game.version+'</div>'+
				
				((researchStr!='' || wrathStr!='' || pledgeStr!='')?(
				'</div><div class="subsection">'+
				'<div class="title">Special</div>'+
				(researchStr!=''?_('<div class="listing"><b>Research :</b> ')+researchStr+_(' remaining</div>'):'')+
				(wrathStr!=''?_('<div class="listing"><b>Grandmatriarchs status :</b> ')+wrathStr+'</div>':'')+
				(pledgeStr!=''?_('<div class="listing"><b>Pledge :</b> ')+pledgeStr+_(' remaining</div>'):'')+
				''
				):'')+
				
				(Game.prestige['Heavenly chips']>0?(
				'</div><div class="subsection">'+
				_('<div class="title">Prestige</div>')+
				_('<div class="listing"><small>(Note : each heavenly chip grants you +2% CpS multiplier. You can gain more chips by resetting with a lot of cookies.)</small></div>')+
				...
				'</div><div class="subsection">'+
				_('<div class="title">Upgrades unlocked</div>')+
				_('<div class="listing"><b>Unlocked :</b> ')+upgradesOwned+'/'+upgradesTotal+' ('+Math.round((upgradesOwned/upgradesTotal)*100)+'%)</div>'+
				'<div class="listing" style="overflow-y:hidden;">'+upgrades+'</div>'+
				(cookieUpgrades!=''?(_('<div class="listing"><b>Cookies</b></div>')+
				'<div class="listing" style="overflow-y:hidden;">'+cookieUpgrades+'</div>'):'')+
				'</div><div class="subsection" style="padding-bottom:128px;">'+
				_('<div class="title">Achievements</div>')+
				_('<div class="listing"><b>Unlocked :</b> ')+achievementsOwned+'/'+achievementsTotal+' ('+Math.round((achievementsOwned/achievementsTotal)*100)+'%)</div>'+
				_('<div class="listing"><b>Milk :</b> ')+Math.round(Game.milkProgress*100)+'% ('+milkName+_(') <small>(Note : you gain milk through achievements. Milk can unlock unique upgrades over time.)</small></div>')+
				'<div class="listing" style="overflow-y:hidden;">'+achievements+'</div>'+
				(shadowAchievements!=''?(
					_('<div class="listing"><b>Shadow achievements</b> <small>(These are feats that are either unfair or difficult to attain. They do not give milk.)</small></div>')+
		...
		Game.getNewTicker=function()
		{
			...
			{				
				...
				_('<q>Moist cookies.</q><sig>grandma</sig>'),
				_('<q>We\'re nice grandmas.</q><sig>grandma</sig>'),
				_('<q>Indentured servitude.</q><sig>grandma</sig>'),
				_('<q>Come give grandma a kiss.</q><sig>grandma</sig>'),
				_('<q>Why don\'t you visit more often?</q><sig>grandma</sig>'),
				_('<q>Call me...</q><sig>grandma</sig>')
				]));
				
				if (Game.Objects['Grandma'].amount>=50) list.push(choose([
				_('<q>Absolutely disgusting.</q><sig>grandma</sig>'),
				_('<q>You make me sick.</q><sig>grandma</sig>'),
				_('<q>You disgust me.</q><sig>grandma</sig>'),
				_('<q>We rise.</q><sig>grandma</sig>'),
				_('<q>It begins.</q><sig>grandma</sig>'),
				_('<q>It\'ll all be over soon.</q><sig>grandma</sig>'),
				_('<q>You could have stopped it.</q><sig>grandma</sig>')
				]));
				
				if (Game.HasAchiev('Just wrong')) list.push(choose([
				_('News : cookie manufacturer downsizes, sells own grandmother!'),
				_('<q>It has betrayed us, the filthy little thing.</q><sig>grandma</sig>'),
				_('<q>It tried to get rid of us, the nasty little thing.</q><sig>grandma</sig>'),
				_('<q>It thought we would go away by selling us. How quaint.</q><sig>grandma</sig>'),
				_('<q>I can smell your rotten cookies.</q><sig>grandma</sig>')
				]));
				
				if (Game.Objects['Grandma'].amount>=1 && Game.pledges>0 && Game.elderWrath==0) list.push(choose([
				_('<q>shrivel</q><sig>grandma</sig>'),
				_('<q>writhe</q><sig>grandma</sig>'),
				_('<q>throb</q><sig>grandma</sig>'),
				_('<q>gnaw</q><sig>grandma</sig>'),
				_('<q>We will rise again.</q><sig>grandma</sig>'),
				_('<q>A mere setback.</q><sig>grandma</sig>'),
				_('<q>We are not satiated.</q><sig>grandma</sig>'),
				_('<q>Too late.</q><sig>grandma</sig>')
				]));
				
				if (Game.Objects['Farm'].amount>0) list.push(choose([
				_('News : cookie farms suspected of employing undeclared elderly workforce!'),
				_('News : cookie farms release harmful chocolate in our rivers, says scientist!'),
				_('News : genetically-modified chocolate controversy strikes cookie farmers!'),
				_('News : free-range farm cookies popular with today\'s hip youth, says specialist.'),
				_('News : farm cookies deemed unfit for vegans, says nutritionist.')
				]));
				
				if (Game.Objects['Factory'].amount>0) list.push(choose([
				_('News : cookie factories linked to global warming!'),
				_('News : cookie factories involved in chocolate weather controversy!'),
				_('News : cookie factories on strike, robotic minions employed to replace workforce!'),
				_('News : cookie factories on strike - workers demand to stop being paid in cookies!'),
				_('News : factory-made cookies linked to obesity, says study.')
				]));
				
				if (Game.Objects['Mine'].amount>0) list.push(choose([
				fmt(_('News : {0} miners dead in chocolate mine catastrophe!'),Math.floor(Math.random()*1000+2)),
				fmt(_('News : {0} miners trapped in collapsed chocolate mine!'),Math.floor(Math.random()*1000+2)),
				_('News : chocolate mines found to cause earthquakes and sink holes!'),
				_('News : chocolate mine goes awry, floods village in chocolate!'),
				_('News : depths of chocolate mines found to house "peculiar, chocolaty beings"!')
				]));
				
				if (Game.Objects['Shipment'].amount>0) list.push(choose([
				_('News : new chocolate planet found, becomes target of cookie-trading spaceships!'),
				_('News : massive chocolate planet found with 99.8% certified pure dark chocolate core!'),
				_('News : space tourism booming as distant planets attract more bored millionaires!'),
				_('News : chocolate-based organisms found on distant planet!'),
				_('News : ancient baking artifacts found on distant planet; "terrifying implications", experts say.')
				]));
				
				if (Game.Objects['Alchemy lab'].amount>0) list.push(choose([
				_('News : national gold reserves dwindle as more and more of the precious mineral is turned to cookies!'),
				_('News : chocolate jewelry found fashionable, gold and diamonds "just a fad", says specialist.'),
				_('News : silver found to also be transmutable into white chocolate!'),
				_('News : defective alchemy lab shut down, found to convert cookies to useless gold.'),
				_('News : alchemy-made cookies shunned by purists!')
				]));
				
				if (Game.Objects['Portal'].amount>0) list.push(choose([
				_('News : nation worried as more and more unsettling creatures emerge from dimensional portals!'),
				_('News : dimensional portals involved in city-engulfing disaster!'),
				_('News : tourism to cookieverse popular with bored teenagers! Casualty rate as high as 73%!'),
				_('News : cookieverse portals suspected to cause fast aging and obsession with baking, says study.'),
				_('News : "do not settle near portals," says specialist; "your children will become strange and corrupted inside."')
				]));
				
				if (Game.Objects['Time machine'].amount>0) list.push(choose([
				_('News : time machines involved in history-rewriting scandal! Or are they?'),
				_('News : time machines used in unlawful time tourism!'),
				_('News : cookies brought back from the past "unfit for human consumption", says historian.'),
				_('News : various historical figures inexplicably replaced with talking lumps of dough!'),
				_('News : "I have seen the future," says time machine operator, "and I do not wish to go there again."')
				]));
				
				if (Game.Objects['Antimatter condenser'].amount>0) list.push(choose([
				_('News : whole town seemingly swallowed by antimatter-induced black hole; more reliable sources affirm town "never really existed"!'),
				_('News : "explain to me again why we need particle accelerators to bake cookies?" asks misguided local woman.'),
				_('News : first antimatter condenser successfully turned on, doesn\'t rip apart reality!'),
				_('News : researchers conclude that what the cookie industry needs, first and foremost, is "more magnets".'),
				_('News : "unravelling the fabric of reality just makes these cookies so much tastier", claims scientist.')
				]));
				
				if (Game.HasAchiev('Base 10')) list.push(_('News : cookie manufacturer completely forgoes common sense, lets OCD drive building decisions!'));
				if (Game.HasAchiev('From scratch')) list.push(_('News : follow the tear-jerking, riches-to-rags story about a local cookie manufacturer who decided to give it all up!'));
				if (Game.HasAchiev('A world filled with cookies')) list.push(_('News : known universe now jammed with cookies! No vacancies!'));
				if (Game.HasAchiev('Serendipity')) list.push(_('News : local cookie manufacturer becomes luckiest being alive!'));
				
				if (Game.Has('Kitten helpers')) list.push(_('News : faint meowing heard around local cookie facilities; suggests new ingredient being tested.'));
				if (Game.Has('Kitten workers')) list.push(_('News : crowds of meowing kittens with little hard hats reported near local cookie facilities.'));
				if (Game.Has('Kitten engineers')) list.push(_('News : surroundings of local cookie facilities now overrun with kittens in adorable little suits. Authorities advise to stay away from the premises.'));
				if (Game.Has('Kitten overseers')) list.push(_('News : locals report troups of bossy kittens meowing adorable orders at passerbys.'));
				
				var animals=[_('newts'),_('penguins'),_('scorpions'),_('axolotls'),_('puffins'),_('porpoises'),_('blowfish'),_('horses'),_('crayfish'),_('slugs'),_('humpback whales'),_('nurse sharks'),_('giant squids'),_('polar bears'),_('fruit bats'),_('frogs'),_('sea squirts'),_('velvet worms'),_('mole rats'),_('paramecia'),_('nematodes'),_('tardigrades'),_('giraffes')];
				if (Game.cookiesEarned>=10000) list.push(
				_('News : ')+choose([
					fmt(_('cookies found to {0} in {1}!'),choose([_('increase lifespan'),_('sensibly increase intelligence'),_('reverse aging'),_('decrease hair loss'),_('prevent arthritis'),_('cure blindness')]),choose(animals)),
					fmt(_('cookies found to make {0} {1}!'),choose(animals),choose([_('more docile'),_('more handsome'),_('nicer'),_('less hungry'),_('more pragmatic'),_('tastier')])),
					fmt(_('cookies tested on {0}, found to have no ill effects.'),choose(animals)),
					fmt(_('cookies unexpectedly popular among {0}!'),choose(animals)),
					fmt(_('unsightly lumps found on {0} near cookie facility; "they\'ve pretty much always looked like that", say biologists.'),choose(animals)),
					fmt(_('new species of {0} discovered in distant country; "yup, tastes like cookies", says biologist.'),choose(animals)),
					fmt(_('cookies go well with roasted {0}, says controversial chef.'),choose(animals)),
					fmt(_('"do your cookies contain {0}?", asks PSA warning against counterfeit cookies.'),choose(animals))
					]),
				fmt(_('News : "{0}", reveals celebrity.'),choose([
					_('I\'m all about cookies'),
					_('I just can\'t stop eating cookies. I think I seriously need help'),
					_('I guess I have a cookie problem'),
					_('I\'m not addicted to cookies. That\'s just speculation by fans with too much free time'),
					_('my upcoming album contains 3 songs about cookies'),
					_('I\'ve had dreams about cookies 3 nights in a row now. I\'m a bit worried honestly'),
					_('accusations of cookie abuse are only vile slander'),
					_('cookies really helped me when I was feeling low'),
					_('cookies are the secret behind my perfect skin'),
					_('cookies helped me stay sane while filming my upcoming movie'),
					_('cookies helped me stay thin and healthy'),
					_('I\'ll say one word, just one : cookies'),
					_('alright, I\'ll say it - I\'ve never eaten a single cookie in my life')
					])),
				_('News : ')+choose([_('doctors recommend twice-daily consumption of fresh cookies.'),_('doctors warn against chocolate chip-snorting teen fad.'),_('doctors advise against new cookie-free fad diet.'),_('doctors warn mothers about the dangers of "home-made cookies".')]),
				choose([
					_('News : scientist predicts imminent cookie-related "end of the world"; becomes joke among peers.'),
					_('News : man robs bank, buys cookies.'),
					_('News : what makes cookies taste so right? "Probably all the [*****] they put in them", says anonymous tipper.'),
					_('News : man found allergic to cookies; "what a weirdo", says family.'),
					_('News : foreign politician involved in cookie-smuggling scandal.'),
					fmt(_('News : cookies now more popular than {0}, says study.'),choose([_('cough drops'),_('broccoli'),_('smoked herring'),_('cheese'),_('video games'),_('stable jobs'),_('relationships'),_('time travel'),_('cat videos'),_('tango'),_('fashion'),_('television'),_('nuclear warfare'),_('whatever it is we ate before'),_('politics'),_('oxygen'),_('lamps')])),
					fmt(_('News : obesity epidemic strikes nation; experts blame {0}.'),choose([_('twerking'),_('that darn rap music'),_('video-games'),_('lack of cookies'),_('mysterious ghostly entities'),_('aliens'),_('parents'),_('schools'),_('comic-books'),_('cookie-snorting fad')])),
					_('News : cookie shortage strikes town, people forced to eat cupcakes; "just not the same", concedes mayor.'),
					_('News : "you gotta admit, all this cookie stuff is a bit ominous", says confused idiot.'),
					_('News : movie cancelled from lack of actors; "everybody\'s at home eating cookies", laments director.'),
					_('News : comedian forced to cancel cookie routine due to unrelated indigestion.'),
					_('News : new cookie-based religion sweeps the nation.'),
					_('News : fossil records show cookie-based organisms prevalent during Cambrian explosion, scientists say.'),
					_('News : mysterious illegal cookies seized; "tastes terrible", says police.'),
					_('News : man found dead after ingesting cookie; investigators favor "mafia snitch" hypothesis.'),
					_('News : "the universe pretty much loops on itself," suggests researcher; "it\'s cookies all the way down."'),
					_('News : minor cookie-related incident turns whole town to ashes; neighboring cities asked to chip in for reconstruction.'),
					_('News : is our media controlled by the cookie industry? This could very well be the case, says crackpot conspiracy theorist.'),
					_('News : ')+choose([_('cookie-flavored popcorn pretty damn popular; "we kinda expected that", say scientists.'),_('cookie-flavored cereals break all known cereal-related records'),_('cookies popular among all age groups, including fetuses, says study.'),_('cookie-flavored popcorn sales exploded during screening of Grandmothers II : The Moistening.')]),
					_('News : all-cookie restaurant opening downtown. Dishes such as braised cookies, cookie thermidor, and for dessert : crepes.'),
					fmt(_('News : cookies could be the key to {0}, say scientists.'),choose([_('eternal life'),_('infinite riches'),_('eternal youth'),_('eternal beauty'),_('curing baldness'),_('world peace'),_('solving world hunger'),_('ending all wars world-wide'),_('making contact with extraterrestrial life'),_('mind-reading'),_('better living'),_('better eating'),_('more interesting TV shows'),_('faster-than-light travel'),_('quantum baking'),_('chocolaty goodness'),_('gooder thoughtness')]))
					])
				);
			}
			...
			if (list.length==0)
			{
				if (Game.cookiesEarned<5) list.push(_('You feel like making cookies. But nobody wants to eat your cookies.'));
				else if (Game.cookiesEarned<50) list.push(_('Your first batch goes to the trash. The neighborhood raccoon barely touches it.'));
				else if (Game.cookiesEarned<100) list.push(_('Your family accepts to try some of your cookies.'));
				else if (Game.cookiesEarned<500) list.push(_('Your cookies are popular in the neighborhood.'));
				else if (Game.cookiesEarned<1000) list.push(_('People are starting to talk about your cookies.'));
				else if (Game.cookiesEarned<3000) list.push(_('Your cookies are talked about for miles around.'));
				else if (Game.cookiesEarned<6000) list.push(_('Your cookies are renowned in the whole town!'));
				else if (Game.cookiesEarned<10000) list.push(_('Your cookies bring all the boys to the yard.'));
				else if (Game.cookiesEarned<20000) list.push(_('Your cookies now have their own website!'));
				else if (Game.cookiesEarned<30000) list.push(_('Your cookies are worth a lot of money.'));
				else if (Game.cookiesEarned<40000) list.push(_('Your cookies sell very well in distant countries.'));
				else if (Game.cookiesEarned<60000) list.push(_('People come from very far away to get a taste of your cookies.'));
				else if (Game.cookiesEarned<80000) list.push(_('Kings and queens from all over the world are enjoying your cookies.'));
				else if (Game.cookiesEarned<100000) list.push(_('There are now museums dedicated to your cookies.'));
				else if (Game.cookiesEarned<200000) list.push(_('A national day has been created in honor of your cookies.'));
				else if (Game.cookiesEarned<300000) list.push(_('Your cookies have been named a part of the world wonders.'));
				else if (Game.cookiesEarned<450000) list.push(_('History books now include a whole chapter about your cookies.'));
				else if (Game.cookiesEarned<600000) list.push(_('Your cookies have been placed under government surveillance.'));
				else if (Game.cookiesEarned<1000000) list.push(_('The whole planet is enjoying your cookies!'));
				else if (Game.cookiesEarned<5000000) list.push(_('Strange creatures from neighboring planets wish to try your cookies.'));
				else if (Game.cookiesEarned<10000000) list.push(_('Elder gods from the whole cosmos have awoken to taste your cookies.'));
				else if (Game.cookiesEarned<30000000) list.push(_('Beings from other dimensions lapse into existence just to get a taste of your cookies.'));
				else if (Game.cookiesEarned<100000000) list.push(_('Your cookies have achieved sentience.'));
				else if (Game.cookiesEarned<300000000) list.push(_('The universe has now turned into cookie dough, to the molecular level.'));
				else if (Game.cookiesEarned<1000000000) list.push(_('Your cookies are rewriting the fundamental laws of the universe.'));
				else if (Game.cookiesEarned<10000000000) list.push(_('A local news station runs a 10-minute segment about your cookies. Success!<br><span style="font-size:50%;">(you win a cookie)</span>'));
				else if (Game.cookiesEarned<10100000000) list.push(_('it\'s time to stop playing'));//only show this for 100 millions (it's funny for a moment)
			}
			...
			if (Game.elderWrath>0 && (Game.pledges==0 || Math.random()<0.5))
			{
				...
				if (Game.elderWrath==1) list.push(choose([
					_('News : millions of old ladies reported missing!'),
					_('News : processions of old ladies sighted around cookie facilities!'),
					_('News : families around the continent report agitated, transfixed grandmothers!'),
					_('News : doctors swarmed by cases of old women with glassy eyes and a foamy mouth!'),
					_('News : nurses report "strange scent of cookie dough" around female elderly patients!')
				]));
				if (Game.elderWrath==2) list.push(choose([
					_('News : town in disarray as strange old ladies break into homes to abduct infants and baking utensils!'),
					_('News : sightings of old ladies with glowing eyes terrify local population!'),
					_('News : retirement homes report "female residents slowly congealing in their seats"!'),
					_('News : whole continent undergoing mass exodus of old ladies!'),
					_('News : old women freeze in place in streets, ooze warm sugary syrup!')
				]));
				if (Game.elderWrath==3) list.push(choose([
					_('News : large "flesh highways" scar continent, stretch between various cookie facilities!'),
					_('News : wrinkled "flesh tendrils" visible from space!'),
					_('News : remains of "old ladies" found frozen in the middle of growing fleshy structures!'), 
					_('News : all hope lost as writhing mass of flesh and dough engulfs whole city!'),
					_('News : nightmare continues as wrinkled acres of flesh expand at alarming speeds!')
				]));
			}
		...
		Game.RebuildStore=function()//redraw the store from scratch
		{
			...
			for (var i in Game.Objects)
			{
				...
				'<div style="min-width:300px;"><div style="float:right;"><span class="price">'+Beautify(Math.round(me.price))+'</span></div><div class="name">'+_(me.name)+'</div>'+_('<small>[owned : ')+me.amount+'</small>]<div class="description">'+me.desc+'</div></div>'
		...
		//define objects
		new Game.Object(_N('Cursor'),_('cursor|cursors|clicked'),_('Autoclicks once every 10 seconds.'),'cursor','cursoricon','',15,function(){
			var add=0;
			if (Game.Has('Thousand fingers')) add+=0.1;
			if (Game.Has('Million fingers')) add+=0.5;
			if (Game.Has('Billion fingers')) add+=2;
			...
			if (!l('rowInfo'+this.id)) l('sectionLeftInfo').innerHTML='<div class="info" id="rowInfo'+this.id+'"><div class="infoContent" id="rowInfoContent'+this.id+'"></div><div><a onclick="Game.ObjectsById['+this.id+'].sell();">'+_('Sell 1')+'</a></div></div>';
		},function(){
		...
		new Game.Object(_N('Grandma'),_('grandma|grandmas|baked'),_('A nice grandma to bake more cookies.'),'grandma','grandmaIcon','grandmaBackground',100,function(){
			var mult=0;
		...
		new Game.Object(_N('Farm'),_('farm|farms|harvested'),_('Grows cookie plants from cookie seeds.'),'farm','farmIcon','farmBackground',500,function(){
		...
		new Game.Object(_N('Factory'),_('factory|factories|mass-produced'),_('Produces large quantities of cookies.'),'factory','factoryIcon','factoryBackground',3000,function(){
		...
		new Game.Object(_N('Mine'),_('mine|mines|mined'),_('Mines out cookie dough and chocolate chips.'),'mine','mineIcon','mineBackground',10000,function(){
		...
		new Game.Object(_N('Shipment'),_('shipment|shipments|shipped'),_('Brings in fresh cookies from the cookie planet.'),'shipment','shipmentIcon','shipmentBackground',40000,function(){
		...
		new Game.Object(_N('Alchemy lab'),_('alchemy lab|alchemy labs|transmuted'),_('Turns gold into cookies!'),'alchemylab','alchemylabIcon','alchemylabBackground',200000,function(){
		...
		new Game.Object(_N('Portal'),_('portal|portals|retrieved'),_('Opens a door to the Cookieverse.'),'portal','portalIcon','portalBackground',1666666,function(){
		...
		new Game.Object(_N('Time machine'),_('time machine|time machines|recovered'),_('Brings cookies from the past, before they were even eaten.'),'timemachine','timemachineIcon','timemachineBackground',123456789,function(){
		...
		new Game.Object(_N('Antimatter condenser'),_('antimatter condenser|antimatter condensers|condensed'),_('Condenses the antimatter in the universe into cookies.'),'antimattercondenser','antimattercondenserIcon','antimattercondenserBackground',3999999999,function(){
		...
		Game.Objects['Antimatter condenser'].displayName='<span style="font-size:65%;">'+_('Antimatter condenser')+'</span>';//shrink the name since it's so large
		...
		Game.RebuildUpgrades=function()//recalculate the upgrades you can buy
		{
			...
			for (var i in Game.UpgradesInStore)
			{
				...
				//'<b>'+_(me.name)+'</b>'+me.desc
				'<div style="min-width:200px;"><div style="float:right;"><span class="price">'+Beautify(Math.round(me.basePrice))+'</span></div><small>[Upgrade]</small><div class="name">'+_(me.name)+'</div><div class="description">'+me.desc+'</div></div>'
		...
		new Game.Upgrade(_N('Reinforced index finger'),_('The mouse gains <b>+1</b> cookie per click.<br>Cursors gain <b>+0.1</b> base CpS.<q>prod prod</q>'),100,[0,0]);
		new Game.Upgrade(_N('Carpal tunnel prevention cream'),_('The mouse and cursors are <b>twice</b> as efficient.'),400,[0,0]);
		new Game.Upgrade(_N('Ambidextrous'),_('The mouse and cursors are <b>twice</b> as efficient.<q>Look ma, both hands!</q>'),10000,[0,6]);
		new Game.Upgrade(_N('Thousand fingers'),_('The mouse and cursors gain <b>+0.1</b> cookies for each non-cursor object owned.<q>clickity</q>'),500000,[0,6]);
		new Game.Upgrade(_N('Million fingers'),_('The mouse and cursors gain <b>+0.5</b> cookies for each non-cursor object owned.<q>clickityclickity</q>'),50000000,[1,6]);
		new Game.Upgrade(_N('Billion fingers'),_('The mouse and cursors gain <b>+2</b> cookies for each non-cursor object owned.<q>clickityclickityclickity</q>'),500000000,[2,6]);
		new Game.Upgrade(_N('Trillion fingers'),_('The mouse and cursors gain <b>+10</b> cookies for each non-cursor object owned.<q>clickityclickityclickityclickity</q>'),5000000000,[3,6]);
		
		order=200;
		new Game.Upgrade(_N('Forwards from grandma'),_('Grandmas gain <b>+0.3</b> base CpS.<q>RE:RE:thought you\'d get a kick out of this ;))</q>'),Game.Objects['Grandma'].basePrice*tier1,[1,0]);
		new Game.Upgrade(_N('Steel-plated rolling pins'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Grandma'].basePrice*tier2,[1,0]);
		new Game.Upgrade(_N('Lubricated dentures'),_('Grandmas are <b>twice</b> as efficient.<q>Squish</q>'),Game.Objects['Grandma'].basePrice*tier3,[1,1]);
		
		order=300;
		new Game.Upgrade(_N('Cheap hoes'),_('Farms gain <b>+0.5</b> base CpS.'),Game.Objects['Farm'].basePrice*tier1,[2,0]);
		new Game.Upgrade(_N('Fertilizer'),_('Farms are <b>twice</b> as efficient.<q>It\'s chocolate, I swear.</q>'),Game.Objects['Farm'].basePrice*tier2,[2,0]);
		new Game.Upgrade(_N('Cookie trees'),_('Farms are <b>twice</b> as efficient.<q>A relative of the breadfruit.</q>'),Game.Objects['Farm'].basePrice*tier3,[2,1]);
		
		order=400;
		new Game.Upgrade(_N('Sturdier conveyor belts'),_('Factories gain <b>+4</b> base CpS.'),Game.Objects['Factory'].basePrice*tier1,[4,0]);
		new Game.Upgrade(_N('Child labor'),_('Factories are <b>twice</b> as efficient.<q>Cheaper, healthier workforce - and so much more receptive to whipping!</q>'),Game.Objects['Factory'].basePrice*tier2,[4,0]);
		new Game.Upgrade(_N('Sweatshop'),_('Factories are <b>twice</b> as efficient.<q>Slackers will be terminated.</q>'),Game.Objects['Factory'].basePrice*tier3,[4,1]);
		
		order=500;
		new Game.Upgrade(_N('Sugar gas'),_('Mines gain <b>+10</b> base CpS.<q>A pink, volatile gas, found in the depths of some chocolate caves.</q>'),Game.Objects['Mine'].basePrice*tier1,[3,0]);
		new Game.Upgrade(_N('Megadrill'),_('Mines are <b>twice</b> as efficient.'),Game.Objects['Mine'].basePrice*tier2,[3,0]);
		new Game.Upgrade(_N('Ultradrill'),_('Mines are <b>twice</b> as efficient.'),Game.Objects['Mine'].basePrice*tier3,[3,1]);
		
		order=600;
		new Game.Upgrade(_N('Vanilla nebulae'),_('Shipments gain <b>+30</b> base CpS.'),Game.Objects['Shipment'].basePrice*tier1,[5,0]);
		new Game.Upgrade(_N('Wormholes'),_('Shipments are <b>twice</b> as efficient.<q>By using these as shortcuts, your ships can travel much faster.</q>'),Game.Objects['Shipment'].basePrice*tier2,[5,0]);
		new Game.Upgrade(_N('Frequent flyer'),_('Shipments are <b>twice</b> as efficient.<q>Come back soon!</q>'),Game.Objects['Shipment'].basePrice*tier3,[5,1]);
		
		order=700;
		new Game.Upgrade(_N('Antimony'),_('Alchemy labs gain <b>+100</b> base CpS.<q>Actually worth a lot of mony.</q>'),Game.Objects['Alchemy lab'].basePrice*tier1,[6,0]);
		new Game.Upgrade(_N('Essence of dough'),_('Alchemy labs are <b>twice</b> as efficient.<q>Extracted through the 5 ancient steps of alchemical baking.</q>'),Game.Objects['Alchemy lab'].basePrice*tier2,[6,0]);
		new Game.Upgrade(_N('True chocolate'),_('Alchemy labs are <b>twice</b> as efficient.<q>The purest form of cacao.</q>'),Game.Objects['Alchemy lab'].basePrice*tier3,[6,1]);
		
		order=800;
		new Game.Upgrade(_N('Ancient tablet'),_('Portals gain <b>+1,666</b> base CpS.<q>A strange slab of peanut brittle, holding an ancient cookie recipe. Neat!</q>'),Game.Objects['Portal'].basePrice*tier1,[7,0]);
		new Game.Upgrade(_N('Insane oatling workers'),_('Portals are <b>twice</b> as efficient.<q>ARISE, MY MINIONS!</q>'),Game.Objects['Portal'].basePrice*tier2,[7,0]);
		new Game.Upgrade(_N('Soul bond'),_('Portals are <b>twice</b> as efficient.<q>So I just sign up and get more cookies? Sure, whatever!</q>'),Game.Objects['Portal'].basePrice*tier3,[7,1]);
		
		order=900;
		new Game.Upgrade(_N('Flux capacitors'),_('Time machines gain <b>+9,876</b> base CpS.<q>Bake to the future.</q>'),1234567890,[8,0]);
		new Game.Upgrade(_N('Time paradox resolver'),_('Time machines are <b>twice</b> as efficient.<q>No more fooling around with your own grandmother!</q>'),9876543210,[8,0]);
		new Game.Upgrade(_N('Quantum conundrum'),_('Time machines are <b>twice</b> as efficient.<q>It\'s full of stars!</q>'),98765456789,[8,1]);
		
		order=20000;
		new Game.Upgrade(_N('Kitten helpers'),_('You gain <b>more CpS</b> the more milk you have.<q>meow may I help you</q>'),9000000,[1,7]);
		new Game.Upgrade(_N('Kitten workers'),_('You gain <b>more CpS</b> the more milk you have.<q>meow meow meow meow</q>'),9000000000,[2,7]);
		
		order=10000;
		type='cookie';power=5;
		new Game.Upgrade(_N('Oatmeal raisin cookies'),_('Cookie production multiplier <b>+5%</b>.<q>No raisin to hate these.</q>'),99999999,[0,3]);
		new Game.Upgrade(_N('Peanut butter cookies'),_('Cookie production multiplier <b>+5%</b>.'),99999999,[1,3]);
		new Game.Upgrade(_N('Plain cookies'),_('Cookie production multiplier <b>+5%</b>.<q>Meh.</q>'),99999999,[2,3]);
		new Game.Upgrade(_N('Coconut cookies'),_('Cookie production multiplier <b>+5%</b>.'),999999999,[3,3]);
		new Game.Upgrade(_N('White chocolate cookies'),_('Cookie production multiplier <b>+5%</b>.'),999999999,[4,3]);
		new Game.Upgrade(_N('Macadamia nut cookies'),_('Cookie production multiplier <b>+5%</b>.'),999999999,[5,3]);
		power=10;new Game.Upgrade(_N('Double-chip cookies'),_('Cookie production multiplier <b>+10%</b>.'),99999999999,[6,3]);
		power=5;new Game.Upgrade(_N('Sugar cookies'),_('Cookie production multiplier <b>+5%</b>.'),99999999,[7,3]);
		power=10;new Game.Upgrade(_N('White chocolate macadamia nut cookies'),_('Cookie production multiplier <b>+10%</b>.'),99999999999,[8,3]);
		new Game.Upgrade(_N('All-chocolate cookies'),_('Cookie production multiplier <b>+10%</b>.'),99999999999,[9,3]);
		type='';power=0;
		
		order=100;
		new Game.Upgrade(_N('Quadrillion fingers'),_('The mouse and cursors gain <b>+20</b> cookies for each non-cursor object owned.<q>clickityclickityclickityclickityclick</q>'),50000000000,[3,6]);
		
		order=200;new Game.Upgrade(_N('Prune juice'),_('Grandmas are <b>twice</b> as efficient.<q>Gets me going.</q>'),Game.Objects['Grandma'].basePrice*tier4,[1,2]);
		order=300;new Game.Upgrade(_N('Genetically-modified cookies'),_('Farms are <b>twice</b> as efficient.<q>All-natural mutations.</q>'),Game.Objects['Farm'].basePrice*tier4,[2,2]);
		order=400;new Game.Upgrade(_N('Radium reactors'),_('Factories are <b>twice</b> as efficient.<q>Gives your cookies a healthy glow.</q>'),Game.Objects['Factory'].basePrice*tier4,[4,2]);
		order=500;new Game.Upgrade(_N('Ultimadrill'),_('Mines are <b>twice</b> as efficient.<q>Pierce the heavens, etc.</q>'),Game.Objects['Mine'].basePrice*tier4,[3,2]);
		order=600;new Game.Upgrade(_N('Warp drive'),_('Shipments are <b>twice</b> as efficient.'),Game.Objects['Shipment'].basePrice*tier4,[5,2]);
		order=700;new Game.Upgrade(_N('Ambrosia'),_('Alchemy labs are <b>twice</b> as efficient.'),Game.Objects['Alchemy lab'].basePrice*tier4,[6,2]);
		order=800;new Game.Upgrade(_N('Sanity dance'),_('Portals are <b>twice</b> as efficient.<q>We can change if we want to.<br>We can leave our brains behind.</q>'),Game.Objects['Portal'].basePrice*tier4,[7,2]);
		order=900;new Game.Upgrade(_N('Causality enforcer'),_('Time machines are <b>twice</b> as efficient.<q>What happened, happened.</q>'),1234567890000,[8,2]);
		
		order=5000;
		new Game.Upgrade(_N('Lucky day'),_('Golden cookies appear <b>twice as often</b> and last <b>twice as long</b>.'),777777777,[10,1]);
		new Game.Upgrade(_N('Serendipity'),_('Golden cookies appear <b>twice as often</b> and last <b>twice as long</b>.'),77777777777,[10,1]);
		
		order=20000;
		new Game.Upgrade(_N('Kitten engineers'),_('You gain <b>more CpS</b> the more milk you have.<q>meow meow meow meow, sir</q>'),9000000000000,[3,7]);
		
		order=10000;
		type='cookie';power=15;
		new Game.Upgrade(_N('Dark chocolate-coated cookies'),_('Cookie production multiplier <b>+15%</b>.'),999999999999,[10,3]);
		new Game.Upgrade(_N('White chocolate-coated cookies'),_('Cookie production multiplier <b>+15%</b>.'),999999999999,[11,3]);
		type='';power=0;
		
		order=250;
		new Game.Upgrade(_N('Farmer grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Farm'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		new Game.Upgrade(_N('Worker grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Factory'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		new Game.Upgrade(_N('Miner grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Mine'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		new Game.Upgrade(_N('Cosmic grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Shipment'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		new Game.Upgrade(_N('Transmuted grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Alchemy lab'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		new Game.Upgrade(_N('Altered grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Portal'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		new Game.Upgrade(_N('Grandmas\' grandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Time machine'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});
		...
		Game.SetResearch=function(what,time)
		{
			if (Game.Upgrades[what])
			{
				...
				Game.Popup(_('Research has begun.'));
			}
		}
		
		new Game.Upgrade(_N('Bingo center/Research facility'),_('Grandma-operated science lab and leisure club.<br>Grandmas are <b>4 times</b> as efficient.<br><b>Regularly unlocks new upgrades</b>.'),100000000000,[11,9],function(){Game.SetResearch('Specialized chocolate chips');});
		new Game.Upgrade(_N('Specialized chocolate chips'),_('[Research]<br>Cookie production multiplier <b>+1%</b>.<q>Computer-designed chocolate chips. Computer chips, if you will.</q>'),10000000000,[0,9],function(){Game.SetResearch('Designer cocoa beans');});
		new Game.Upgrade(_N('Designer cocoa beans'),_('[Research]<br>Cookie production multiplier <b>+2%</b>.<q>Now more aerodynamic than ever!</q>'),20000000000,[1,9],function(){Game.SetResearch('Ritual rolling pins');});
		new Game.Upgrade(_N('Ritual rolling pins'),_('[Research]<br>Grandmas are <b>twice</b> as efficient.<q>The result of years of scientific research!</q>'),40000000000,[2,9],function(){Game.SetResearch('Underworld ovens');});
		new Game.Upgrade(_N('Underworld ovens'),_('[Research]<br>Cookie production multiplier <b>+3%</b>.<q>Powered by science, of course!</q>'),80000000000,[3,9],function(){Game.SetResearch('One mind');});
		new Game.Upgrade(_N('One mind'),_('[Research]<br>Each grandma gains <b>+1 base CpS for each 50 grandmas</b>.<div class="warning">Note : the grandmothers are growing restless. Do not encourage them.</div><q>We are one. We are many.</q>'),160000000000,[4,9],function(){Game.elderWrath=1;Game.SetResearch('Exotic nuts');});
		...
		new Game.Upgrade(_N('Exotic nuts'),_('[Research]<br>Cookie production multiplier <b>+4%</b>.<q>You\'ll go crazy over these!</q>'),320000000000,[5,9],function(){Game.SetResearch('Communal brainsweep');});
		new Game.Upgrade(_N('Communal brainsweep'),_('[Research]<br>Each grandma gains another <b>+1 base CpS for each 50 grandmas</b>.<div class="warning">Note : proceeding any further in scientific research may have unexpected results. You have been warned.</div><q>We fuse. We merge. We grow.</q>'),640000000000,[6,9],function(){Game.elderWrath=2;Game.SetResearch('Arcane sugar');});
		new Game.Upgrade(_N('Arcane sugar'),_('[Research]<br>Cookie production multiplier <b>+5%</b>.<q>Tastes like insects, ligaments, and molasses.</q>'),1280000000000,[7,9],function(){Game.SetResearch('Elder Pact');});
		new Game.Upgrade(_N('Elder Pact'),_('[Research]<br>Each grandma gains <b>+1 base CpS for each 20 portals</b>.<div class="warning">Note : this is a bad idea.</div><q>squirm crawl slither writhe<br>today we rise</q>'),2560000000000,[8,9],function(){Game.elderWrath=3;});
		new Game.Upgrade(_N('Elder Pledge'),_('[Repeatable]<br>Contains the wrath of the elders, at least for a while.'),1,[9,9],function()
		{
			Game.elderWrath=0;
			Game.pledges++;
		...
		order=150;
		new Game.Upgrade(_N('Plastic mouse'),_('Clicking gains <b>+1% of your CpS</b>.'),50000,[11,0]);
		new Game.Upgrade(_N('Iron mouse'),_('Clicking gains <b>+1% of your CpS</b>.'),5000000,[11,0]);
		new Game.Upgrade(_N('Titanium mouse'),_('Clicking gains <b>+1% of your CpS</b>.'),500000000,[11,1]);
		new Game.Upgrade(_N('Adamantium mouse'),_('Clicking gains <b>+1% of your CpS</b>.'),50000000000,[11,2]);
		
		order=40000;
		new Game.Upgrade(_N('Ultrascience'),_('Research takes only <b>5 seconds</b>.'),7,[9,2]);//debug purposes only
		Game.Upgrades['Ultrascience'].hide=3;
		
		order=10000;
		type='cookie';power=15;
		new Game.Upgrade(_N('Eclipse cookies'),_('Cookie production multiplier <b>+15%</b>.<q>Look to the cookie.</q>'),9999999999999,[0,4]);
		new Game.Upgrade(_N('Zebra cookies'),_('Cookie production multiplier <b>+15%</b>.'),9999999999999,[1,4]);
		type='';power=0;
		
		order=100;
		new Game.Upgrade(_N('Quintillion fingers'),_('The mouse and cursors gain <b>+100</b> cookies for each non-cursor object owned.<q>man, just go click click click click click, it\'s real easy, man.</q>'),50000000000000,[3,6]);
		
		order=40000;
		new Game.Upgrade(_N('Gold hoard'),_('Golden cookies appear <b>really often</b>.'),7,[10,1]);//debug purposes only
		Game.Upgrades['Gold hoard'].hide=3;
		
		order=15000;
		new Game.Upgrade(_N('Elder Covenant'),_('[Switch]<br>Puts a permanent end to the elders\' wrath, at the price of 5% of your CpS.'),6666666666666,[8,9],function()
		{
			Game.pledgeT=0;
		...
		new Game.Upgrade(_N('Revoke Elder Covenant'),_('[Switch]<br>You will get 5% of your CpS back, but the grandmatriarchs will return.'),6666666666,[8,9],function()
		{
		...
		order=5000;
		new Game.Upgrade(_N('Get lucky'),_('Golden cookie effects last <b>twice as long</b>.<q>You\'ve been up all night, haven\'t you?</q>'),77777777777777,[10,1]);
		
		order=15000;
		new Game.Upgrade(_N('Sacrificial rolling pins'),_('Elder pledge last <b>twice</b> as long.'),2888888888888,[2,9]);
		
		order=10000;
		type='cookie';power=15;
		new Game.Upgrade(_N('Snickerdoodles'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[2,4]);
		new Game.Upgrade(_N('Stroopwafels'),_('Cookie production multiplier <b>+15%</b>.<q>If it ain\'t dutch, it ain\'t much.</q>'),99999999999999,[3,4]);
		new Game.Upgrade(_N('Macaroons'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[4,4]);
		type='';power=0;
		
		order=40000;
		new Game.Upgrade(_N('Neuromancy'),_('Can toggle upgrades on and off at will in the stats menu.'),7,[4,9]);//debug purposes only
		Game.Upgrades['Neuromancy'].hide=3;
		
		order=10000;
		type='cookie';power=15;
		new Game.Upgrade(_N('Empire biscuits'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[5,4]);
		new Game.Upgrade(_N('British tea biscuits'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[6,4]);
		new Game.Upgrade(_N('Chocolate british tea biscuits'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[7,4]);
		new Game.Upgrade(_N('Round british tea biscuits'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[8,4]);
		new Game.Upgrade(_N('Round chocolate british tea biscuits'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[9,4]);
		new Game.Upgrade(_N('Round british tea biscuits with heart motif'),_('Cookie production multiplier <b>+15%</b>.'),99999999999999,[10,4]);
		new Game.Upgrade(_N('Round chocolate british tea biscuits with heart motif'),_('Cookie production multiplier <b>+15%</b>.<q>Quite.</q>'),99999999999999,[11,4]);
		type='';power=0;
		
		
		order=1000;
		new Game.Upgrade(_N('Sugar bosons'),_('Antimatter condensers gain <b>+99,999</b> base CpS.'),Game.Objects['Antimatter condenser'].basePrice*tier1,[13,0]);
		new Game.Upgrade(_N('String theory'),_('Antimatter condensers are <b>twice</b> as efficient.'),Game.Objects['Antimatter condenser'].basePrice*tier2,[13,0]);
		new Game.Upgrade(_N('Large macaron collider'),_('Antimatter condensers are <b>twice</b> as efficient.<q>How singular!</q>'),Game.Objects['Antimatter condenser'].basePrice*tier3,[13,1]);
		new Game.Upgrade(_N('Big bang bake'),_('Antimatter condensers are <b>twice</b> as efficient.<q>And that\'s how it all began.</q>'),Game.Objects['Antimatter condenser'].basePrice*tier4,[13,2]);

		order=250;
		new Game.Upgrade(_N('Antigrandmas'),_('Grandmas are <b>twice</b> as efficient.'),Game.Objects['Antimatter condenser'].basePrice*tier2,[10,9],function(){Game.Objects['Grandma'].drawFunction();});

		order=10000;
		type='cookie';power=20;
		new Game.Upgrade(_N('Madeleines'),_('Cookie production multiplier <b>+20%</b>.<q>Unforgettable!</q>'),199999999999999,[12,3]);
		new Game.Upgrade(_N('Palmiers'),_('Cookie production multiplier <b>+20%</b>.'),199999999999999,[13,3]);
		new Game.Upgrade(_N('Palets'),_('Cookie production multiplier <b>+20%</b>.'),199999999999999,[12,4]);
		new Game.Upgrade(_N('Sabl&eacute;s'),_('Cookie production multiplier <b>+20%</b>.'),199999999999999,[13,4]);
		type='';power=0;
		
		order=20000;
		new Game.Upgrade(_N('Kitten overseers'),_('You gain <b>more CpS</b> the more milk you have.<q>my purrpose is to serve you, sir</q>'),900000000000000,[8,7]);
		
		/*
		new Game.Upgrade(_N('Plain milk'),_('Unlocks <b>plain milk</b>, available in the menu.'),120000000000,[4,8]);
		new Game.Upgrade(_N('Chocolate milk'),_('Unlocks <b>chocolate milk</b>, available in the menu.'),120000000000,[5,8]);
		new Game.Upgrade(_N('Raspberry milk'),_('Unlocks <b>raspberry milk</b>, available in the menu.'),120000000000,[6,8]);
		new Game.Upgrade(_N('Ain\'t got milk'),_('Unlocks <b>no milk please</b>, available in the menu.'),120000000000,[0,8]);
		
		new Game.Upgrade(_N('Blue background'),_('Unlocks the <b>blue background</b>, available in the menu.'),120000000000,[0,9]);
		new Game.Upgrade(_N('Red background'),_('Unlocks the <b>red background</b>, available in the menu.'),120000000000,[1,9]);
		new Game.Upgrade(_N('White background'),_('Unlocks the <b>white background</b>, available in the menu.'),120000000000,[2,9]);
		new Game.Upgrade(_N('Black background'),_('Unlocks the <b>black background</b>, available in the menu.'),120000000000,[3,9]);
		*/
			...
			if (typeof what==='string')
			{
				if (Game.Achievements[what])
				{
					if (Game.Achievements[what].won==0)
					{
						...
						Game.Popup(_('Achievement unlocked :<br>')+_(Game.Achievements[what].name)+'<br> ');
		...
		var order=100;//this is used to set the order in which the items are listed
		//new Game.Achievement(_N('name'),_('description'),[0,0]);
		Game.moneyAchievs=[
		_N('Wake and bake'),				1,
		_N('Making some dough'),			100,
		_N('So baked right now'),			1000,
		_N('Fledgling bakery'),				10000,
		_N('Affluent bakery'),				100000,
		_N('World-famous bakery'),			1000000,
		_N('Cosmic bakery'),				10000000,
		_N('Galactic bakery'),				100000000,
		_N('Universal bakery'), 			1000000000,
		_N('Timeless bakery'), 				5000000000,
		_N('Infinite bakery'), 				10000000000,
		_N('Immortal bakery'), 				50000000000,
		_N('You can stop now'), 			100000000000,
		_N('Cookies all the way down'), 	500000000000,
		_N('Overdose'), 					1000000000000,
		_N('How?'),							10000000000000
		];
		for (var i=0;i<Game.moneyAchievs.length/2;i++)
		{
			var pic=[Math.min(10,i),5];
			if (i==15) pic=[11,5];
			new Game.Achievement(Game.moneyAchievs[i*2],fmt(_('Bake <b>{0}</b> cookie{1}.'),Beautify(Game.moneyAchievs[i*2+1]),(Game.moneyAchievs[i*2+1]==1?'':_('s'))),pic,2);
		}
		
		order=200;
		Game.cpsAchievs=[
		_N('Casual baking'),				1,
		_N('Hardcore baking'),				10,
		_N('Steady tasty stream'),			100,
		_N('Cookie monster'),				1000,
		_N('Mass producer'),				10000,
		_N('Cookie vortex'),				100000,
		_N('Cookie pulsar'),				1000000,
		_N('Cookie quasar'),				10000000,
		_N('A world filled with cookies'),	100000000,
		_N('Let\'s never bake again'),		1000000000
		];
		for (var i=0;i<Game.cpsAchievs.length/2;i++)
		{
			var pic=[i,5];
			new Game.Achievement(Game.cpsAchievs[i*2],fmt(_('Bake <b>{0}</b> cookie{1} per second.'),Beautify(Game.cpsAchievs[i*2+1]),(Game.cpsAchievs[i*2+1]==1?'':_('s'))),pic,2);
		}
		
		order=30000;
		new Game.Achievement(_N('Sacrifice'),_('Reset your game with <b>1 million</b> cookies baked.<q>Easy come, easy go.</q>'),[11,6],2);
		new Game.Achievement(_N('Oblivion'),_('Reset your game with <b>1 billion</b> cookies baked.<q>Back to square one.</q>'),[11,6],2);
		new Game.Achievement(_N('From scratch'),_('Reset your game with <b>1 trillion</b> cookies baked.<q>It\'s been fun.</q>'),[11,6],2);
		
		order=31000;
		new Game.Achievement(_N('Neverclick'),_('Make <b>1 million</b> cookies by only having clicked <b>15 times</b>.'),[12,0],3);
		order=1000;
		new Game.Achievement(_N('Clicktastic'),_('Make <b>1,000</b> cookies from clicking.'),[11,0]);
		new Game.Achievement(_N('Clickathlon'),_('Make <b>100,000</b> cookies from clicking.'),[11,1]);
		new Game.Achievement(_N('Clickolympics'),_('Make <b>10,000,000</b> cookies from clicking.'),[11,1]);
		new Game.Achievement(_N('Clickorama'),_('Make <b>1,000,000,000</b> cookies from clicking.'),[11,2]);
		
		order=1050;
		new Game.Achievement(_N('Click'),_('Have <b>1</b> cursor.'),[0,0]);
		new Game.Achievement(_N('Double-click'),_('Have <b>2</b> cursors.'),[0,6]);
		new Game.Achievement(_N('Mouse wheel'),_('Have <b>50</b> cursors.'),[1,6]);
		new Game.Achievement(_N('Of Mice and Men'),_('Have <b>100</b> cursors.'),[2,6]);
		new Game.Achievement(_N('The Digital'),_('Have <b>200</b> cursors.'),[3,6]);
		
		order=1100;
		new Game.Achievement(_N('Just wrong'),_('Sell a grandma.<q>I thought you loved me.</q>'),[10,9],2);
		new Game.Achievement(_N('Grandma\'s cookies'),_('Have <b>1</b> grandma.'),[1,0]);
		new Game.Achievement(_N('Sloppy kisses'),_('Have <b>50</b> grandmas.'),[1,1]);
		new Game.Achievement(_N('Retirement home'),_('Have <b>100</b> grandmas.'),[1,2]);
		
		order=1200;
		new Game.Achievement(_N('My first farm'),_('Have <b>1</b> farm.'),[2,0]);
		new Game.Achievement(_N('Reap what you sow'),_('Have <b>50</b> farms.'),[2,1]);
		new Game.Achievement(_N('Farm ill'),_('Have <b>100</b> farms.'),[2,2]);
		
		order=1300;
		new Game.Achievement(_N('Production chain'),_('Have <b>1</b> factory.'),[4,0]);
		new Game.Achievement(_N('Industrial revolution'),_('Have <b>50</b> factories.'),[4,1]);
		new Game.Achievement(_N('Global warming'),_('Have <b>100</b> factories.'),[4,2]);
		
		order=1400;
		new Game.Achievement(_N('You know the drill'),_('Have <b>1</b> mine.'),[3,0]);
		new Game.Achievement(_N('Excavation site'),_('Have <b>50</b> mines.'),[3,1]);
		new Game.Achievement(_N('Hollow the planet'),_('Have <b>100</b> mines.'),[3,2]);
		
		order=1500;
		new Game.Achievement(_N('Expedition'),_('Have <b>1</b> shipment.'),[5,0]);
		new Game.Achievement(_N('Galactic highway'),_('Have <b>50</b> shipments.'),[5,1]);
		new Game.Achievement(_N('Far far away'),_('Have <b>100</b> shipments.'),[5,2]);
		
		order=1600;
		new Game.Achievement(_N('Transmutation'),_('Have <b>1</b> alchemy lab.'),[6,0]);
		new Game.Achievement(_N('Transmogrification'),_('Have <b>50</b> alchemy labs.'),[6,1]);
		new Game.Achievement(_N('Gold member'),_('Have <b>100</b> alchemy labs.'),[6,2]);
		
		order=1700;
		new Game.Achievement(_N('A whole new world'),_('Have <b>1</b> portal.'),[7,0]);
		new Game.Achievement(_N('Now you\'re thinking'),_('Have <b>50</b> portals.'),[7,1]);
		new Game.Achievement(_N('Dimensional shift'),_('Have <b>100</b> portals.'),[7,2]);
		
		order=1800;
		new Game.Achievement(_N('Time warp'),_('Have <b>1</b> time machine.'),[8,0]);
		new Game.Achievement(_N('Alternate timeline'),_('Have <b>50</b> time machines.'),[8,1]);
		new Game.Achievement(_N('Rewriting history'),_('Have <b>100</b> time machines.'),[8,2]);
		
		order=7000;
		new Game.Achievement(_N('One with everything'),_('Have <b>at least 1</b> of every building.'),[4,6],2);
		new Game.Achievement(_N('Mathematician'),_('Have at least <b>1 time machine, 2 portals, 4 alchemy labs, 8 shipments</b> and so on (128 max).'),[7,6],2);
		new Game.Achievement(_N('Base 10'),_('Have at least <b>10 time machines, 20 portals, 30 alchemy labs, 40 shipments</b> and so on.'),[8,6],2);
		
		order=10000;
		new Game.Achievement(_N('Golden cookie'),_('Click a <b>golden cookie</b>.'),[10,1],1);
		new Game.Achievement(_N('Lucky cookie'),_('Click <b>7 golden cookies</b>.'),[10,1],1);
		new Game.Achievement(_N('A stroke of luck'),_('Click <b>27 golden cookies</b>.'),[10,1],1);
		
		order=30200;
		new Game.Achievement(_N('Cheated cookies taste awful'),_('Hack in some cookies.'),[10,6],3);
		order=30001;
		new Game.Achievement(_N('Uncanny clicker'),_('Click really, really fast.<q>Well I\'ll be!</q>'),[12,0],2);
		
		order=5000;
		new Game.Achievement(_N('Builder'),_('Own <b>100</b> buildings.'),[4,6],1);
		new Game.Achievement(_N('Architect'),_('Own <b>400</b> buildings.'),[5,6],1);
		order=6000;
		new Game.Achievement(_N('Enhancer'),_('Purchase <b>20</b> upgrades.'),[9,0],1);
		new Game.Achievement(_N('Augmenter'),_('Purchase <b>50</b> upgrades.'),[9,1],1);
		
		order=11000;
		new Game.Achievement(_N('Cookie-dunker'),_('Dunk the cookie.<q>You did it!</q>'),[4,7],2);
		
		order=10000;
		new Game.Achievement(_N('Fortune'),_('Click <b>77 golden cookies</b>.<q>You should really go to bed.</q>'),[10,1],1);
		order=31000;
		new Game.Achievement(_N('True Neverclick'),_('Make <b>1 million</b> cookies with <b>no</b> cookie clicks.<q>This kinda defeats the whole purpose, doesn\'t it?</q>'),[12,0],3);
		
		order=20000;
		new Game.Achievement(_N('Elder nap'),_('Appease the grandmatriarchs at least <b>once</b>.<q>we<br>are<br>eternal</q>'),[8,9],2);
		new Game.Achievement(_N('Elder slumber'),_('Appease the grandmatriarchs at least <b>5 times</b>.<q>our mind<br>outlives<br>the universe</q>'),[8,9],2);
		
		order=1100;
		new Game.Achievement(_N('Elder'),_('Own every grandma type.'),[10,9],2);
		
		order=20000;
		new Game.Achievement(_N('Elder calm'),_('Declare a covenant with the grandmatriarchs.<q>we<br>have<br>fed</q>'),[8,9],2);
		
		order=5000;
		new Game.Achievement(_N('Engineer'),_('Own <b>800</b> buildings.'),[6,6],1);
		
		order=10000;
		new Game.Achievement(_N('Leprechaun'),_('Click <b>777 golden cookies</b>.'),[10,1],1);
		new Game.Achievement(_N('Black cat\'s paw'),_('Click <b>7777 golden cookies</b>.'),[10,1],3);
		
		order=30000;
		new Game.Achievement(_N('Nihilism'),_('Reset your game with <b>1 quadrillion</b> cookies baked.<q>There are many things<br>that need to be erased</q>'),[11,6],2);
		//new Game.Achievement(_N('Galactus\' Reprimand'),_('Reset your game with <b>1 quintillion</b> coo- okay no I')m yanking your chain
		
		order=1900;
		new Game.Achievement(_N('Antibatter'),_('Have <b>1</b> antimatter condenser.'),[13,0]);
		new Game.Achievement(_N('Quirky quarks'),_('Have <b>50</b> antimatter condensers.'),[13,1]);
		new Game.Achievement(_N('It does matter!'),_('Have <b>100</b> antimatter condensers.'),[13,2]);
		
		order=6000;
		new Game.Achievement(_N('Upgrader'),_('Purchase <b>100</b> upgrades.'),[9,2],1);
		
		order=7000;
		new Game.Achievement(_N('Centennial'),_('Have at least <b>100 of everything</b>.'),[9,6],2);
	...
	Game.Logic=function()
	{
		...
		if (Game.researchT==0 && Game.nextResearch)
		{
			...
			Game.Popup(_('Researched : ')+_(Game.UpgradesById[Game.nextResearch].name));
			Game.nextResearch=0;
			Game.researchT=-1;
		}
		...
		if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
			...
			//if (Game.Objects['Factory'].amount>=50 && Game.Objects['Factory'].specialUnlocked==0) {Game.Objects['Factory'].unlockSpecial();Game.Popup(_('You have unlocked the factory dungeons!'));}
	...
	Game.Draw=function()
	{
		...
		var unit=(Math.round(Game.cookiesd)==1?_(' cookie'):_(' cookies'));
		if (Math.round(Game.cookiesd).toString().length>11) unit=_('<br>cookies');
		l('cookies').innerHTML=Beautify(Math.round(Game.cookiesd))+unit+_('<div style="font-size:50%;">per second : ')+Beautify(Game.cookiesPs,1)+'</div>';//display cookie amount
		...
		*/
		...
		for (var i in Game.Objects)
		{
			...
			//update object info
			if (l('rowInfo'+me.id) && Game.T%5==0) l('rowInfoContent'+me.id).innerHTML=fmt(_('&bull; {0} {1}<br>&bull; producing {2} {3} per second<br>&bull; total : {4} {5} {6}'),me.amount,(me.amount==1?me.single:me.plural),Beautify(me.storedTotalCps,1),(me.storedTotalCps==1?_('cookie'):_('cookies')),Beautify(me.totalCookies),(Math.floor(me.totalCookies)==1?_('cookie'):_('cookies')),me.actionName);
		}
